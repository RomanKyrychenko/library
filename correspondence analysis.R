...
gfk <- read.spss("data/Dataset.sav", to.data.frame = T, reencode = "utf-8")
...
ggplot() +
  geom_point(aes(scale(unname(crsp$row$coord[, 1])), scale(unname(crsp$row$coord[, 2])), size = unname(crsp$call$marge.row)), color = "red") +
  geom_text(aes(scale(unname(crsp$row$coord[, 1])), scale(unname(crsp$row$coord[, 2])) + 0.15, label = names(crsp$call$marge.row)), size = 3) +
  geom_point(aes(scale(unname(crsp$col$coord[, 1])), scale(unname(crsp$col$coord[, 2])), size = unname(crsp$call$marge.col)), color = "blue") +
