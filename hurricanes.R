...
#               fill = "#252525", colour = "#252525", size = 0.05) +
#  geom_point(data = hurdat %>% group_by(id) %>% 
#               mutate(time = round(as.numeric(date - min(date))/(3600*6))*6) %>% 
#               filter(maximum_sustained_wind_knots > 0), 
#            aes(longitude, latitude, group = id, 
#                alpha = maximum_sustained_wind_knots*1.852, 
#                size = maximum_sustained_wind_knots*1.852), 
#            stroke = 0, color = "#ece7f2") +
#  scale_size_continuous(range = c(0.0001, 2), guide = guide_legend(title = "Wind speed km/h:")) +
#  scale_alpha_continuous(guide = guide_legend(title = "Wind speed km/h:")) +
...
#
#anim_save("images/hurricanes.gif", animation = g)
