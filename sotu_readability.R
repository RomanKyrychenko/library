...
ggplot(readability, aes(date, Flesch)) +
  ...
  geom_text(
    data = readability %>%
      group_by(president, term) %>%
      summarise(date = mean(date)), aes(date, 0, label = president),
    size = 2.5, alpha = 0.5, family = "Lato",
    nudge_y = 0.05,
    hjust = 0, # hjust = 0,
    angle = 90
  ) +
  annotate("text", x = lubridate::ymd("1792-01-01"), y = 62, label = "easier →", angle = 90, family = "Lato", alpha = 0.5) +
  annotate("text", x = lubridate::ymd("1792-01-01"), y = 45, label = "← more difficult", angle = 90, family = "Lato", alpha = 0.5) +
  scale_x_date(expand = c(0, 0, 0, 0)) +
  scale_y_continuous(
    expand = c(0, 0, 0, 0),
    breaks = c(0, 30, 50, 60, 70),
  ...
  labs(
    title = "As simple as that: how US presidents speak",
    subtitle = "State of the Union readability analysis shows that modern presidents speak easier than their nineteenth-century counterparts.

The State of the Union Address is an annual message delivered by the President of the United States to a joint session of the United States 
Congress at the beginning of each calendar year in office. The message typically includes a budget message and an economic report of the nation, 
and also allows the President to propose a legislative agenda and national priorities.",
    caption = "Data: http://stateoftheunion.onetwothree.net/"
  ) +
  ylab("Flesch readability score") +
